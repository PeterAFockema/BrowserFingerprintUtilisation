...
battery_and_screen_increment = 0
...
def we_view_the_firefox_page_with_some_battery_and_screen_and_webgl_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension_list(["battery", "screen", "webgl"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 

@when('we view the Firefox page with some battery and screen and webRTC values interference')
def we_view_the_firefox_page_with_some_battery_and_screen_and_webRTC_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension_list(["battery", "screen", "webRTC"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 
...
def we_have_a_firefox_page_which_ran_the_battery_and_screen_and_other_features_response(context):
    assert test_manager.html_puller_firefox.html_source != "<html></html>"
...
@then('we will log the battery_and_screen and webRTC testing time in the saved file')
...
def the_firefox_visitor_id_for_battery_and_screen_and_other_features_has_been_recorded(context):
