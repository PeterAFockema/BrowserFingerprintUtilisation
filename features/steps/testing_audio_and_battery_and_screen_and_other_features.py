...
audio_and_battery_and_screen_increment = 0

'''
The following definitions relate to the Firefox browser.
'''

@when('we view the Firefox page with some audio and battery and screen and webgl values interference')
def we_view_the_firefox_page_with_some_audio_and_battery_and_screen_and_webgl_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "battery", "screen",  "webgl"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 

@when('we view the Firefox page with some audio and battery and screen and webRTC values interference')
def we_view_the_firefox_page_with_some_audio_and_battery_and_screen_and_webRTC_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "battery", "screen",  "webRTC"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 
...
@then('we will log the audio_and_battery and screen and webRTC testing time in the saved file')
...
@then('the visitor id for audio and battery and screen and webgl is saved')
