...
audio_and_battery_and_webgl_increment = 0
...
def we_view_the_firefox_page_with_some_audio_and_battery_and_webgl_and_webRTC_values_interference(context):
    ...
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "battery", "webgl",  "webRTC"]), "html.parser").find())
...
@then('we will log the audio_and_battery and webgl and webRTC testing time in the saved file')
def we_will_log_the_audio_and_battery_and_webgl_and_webRTC_time_in_the_saved_file(context):
    assert test_manager.html_puller_firefox.log_time_in_save_file("audio and battery and webgl and webRTC")
