...
@when('we view the Firefox page with some audio and navigator and screen values interference')
def we_view_the_firefox_page_with_some_audio_and_navigator_and_screen_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "navigator", "screen"]), "html.parser").find())
...
def we_view_the_firefox_page_with_some_audio_and_navigator_and_webgl_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "navigator", "webgl"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 
...
def we_view_the_firefox_page_with_some_audio_and_navigator_and_webRTC_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "navigator", "webRTC"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 

@then('we have a Firefox page which ran the audio and navigator and screen response')
@then('we have a Firefox page which ran the audio and navigator and webgl response')
@then('we have a Firefox page which ran the audio and navigator and webRTC response')
def we_have_a_firefox_page_which_ran_the_audio_and_navigator_and_other_features_response(context):
    assert test_manager.html_puller_firefox.html_source != "<html></html>"
...
@then('we will log the audio_and_navigator and webRTC testing time in the saved file')
...
def the_firefox_visitor_id_for_audio_and_navigator_and_other_features_has_been_recorded(context):
