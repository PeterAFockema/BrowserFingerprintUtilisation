...
def we_view_the_firefox_page_with_some_audio_and_battery_and_canvas_and_screen_values_interference(context):
    ...
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "battery", "canvas", "navigator", "screen"]), "html.parser").find())
...
def we_view_the_firefox_page_with_some_audio_and_battery_and_canvas_and_webgl_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "battery", "canvas", "navigator",  "webgl"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 
...
def we_view_the_firefox_page_with_some_audio_and_battery_and_canvas_and_webRTC_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "battery", "canvas", "navigator", "webRTC"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 
