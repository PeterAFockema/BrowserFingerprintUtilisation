...
def we_view_the_firefox_page_with_some_audio_and_canvas_and_screen_and_webgl_values_interference(context):
    ...
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "canvas", "screen",  "webgl"]), "html.parser").find())
...
def we_view_the_firefox_page_with_some_audio_and_canvas_and_screen_and_webRTC_values_interference(context):
    html_puller_firefox = test_manager.html_puller_firefox
    bool(BeautifulSoup(html_puller_firefox.pull_HTML_page_with_extension(["audio", "canvas", "screen",  "webRTC"]), "html.parser").find())
    assert test_manager.html_puller_firefox.html_source != "<html></html>" 
